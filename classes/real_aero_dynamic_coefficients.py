...
class RealAerodynamicCoefficients:
    ...
    def __init__(self, csv_path=r'excels/Coeficientes que vi 2 casas.csv'):
        """
        ...
        # Observe que a tabela de coeficientes está esrcito "Match" errôneamente
        print(
            f"  Faixa de Mach: {self.df['Match'].min():.2f} a {self.df['Match'].max():.2f}")
        ...
        # Grid de alpha: -10 a +10 graus (Escolha para otimizar,
        # funciona adequadamente na faixa que estudaremos,
        # possível problema para disparos semi-vertiais)
        ...
        for col in coeff_names:
            if col in self.df.columns:
                self.splines_1d[col] = interp1d(
                    ...
                    kind='cubic',
                    bounds_error=False,
                    fill_value=(self.df[col].values[0],
                                self.df[col].values[-1])
                )
        ...
        mach_mesh, alpha_mesh = np.meshgrid(
            self.mach_grid, self.alpha_grid, indexing='ij')

        print(
            f"  Grid: {len(self.mach_grid)} Machs x {len(self.alpha_grid)} alphas")
        print(
            f"  Range de alpha: [{np.degrees(self.alpha_grid[0]):.1f}°,\
            {np.degrees(self.alpha_grid[-1]):.1f}°]")
        ...
        for i, _m in enumerate(self.mach_grid):
        ...
        self.grid_2d['CD_total'] = cx_total * \
            cos_alpha_mesh - (cna_grid*sin_alpha_2_mesh)
