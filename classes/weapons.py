...
class Weapons:
    ...
    def get_absolute_position(self, time=0.0):
        ...
        if self.mounted_on_vessel is None:
            ...
            vessel_center_x = (
                vessel_bounds['x_min'] + vessel_bounds['x_max']) / 2
            vessel_center_z = (
                vessel_bounds['z_min'] + vessel_bounds['z_max']) / 2
    ...
    def calculate_firing_angles(self):
        ...
        phi0 = np.arcsin(np.sin(e) / np.cos(theta0)
                         ) if np.cos(theta0) != 0 else np.pi/2
    ...
    def get_info(self):
        ...
        if self.mounted_on_vessel is None:
            info += f"Posição (x, y, z): ({self.position[0]:.1f},\
                {self.position[1]:.1f}, {self.position[2]:.1f}) m\n"
            info += "Montada em: Terra (velocidade = 0)\n"
        else:
            abs_pos = self.get_absolute_position()
            vessel_vel = self.get_velocity()
            info += f"Posição relativa (x, y, z): ({self.position[0]:.1f},\
                {self.position[1]:.1f}, {self.position[2]:.1f}) m\n"
            info += f"Posição absoluta (x, y, z): ({abs_pos[0]:.1f},\
                {abs_pos[1]:.1f}, {abs_pos[2]:.1f}) m\n"
            info += f"Montada em: {self.mounted_on_vessel.name}\n"
            info += f"Velocidade da plataforma: ({vessel_vel[0]:.1f},\
                {vessel_vel[1]:.1f}, {vessel_vel[2]:.1f}) m/s\n"
