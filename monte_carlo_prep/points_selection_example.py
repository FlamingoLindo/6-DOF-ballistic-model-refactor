...
def points_selec_eg():
    ...
    excel_path = 'excels/azimutes/azimutes_otimos_deriva_zero.xlsx'
    ...
    print(
        f"Faixa de elevação: {df_filtrado['Elevacao_deg'].max():.1f}°\
              a {df_filtrado['Elevacao_deg'].min():.1f}°")
    print(
        f"Faixa de alcance: {df_filtrado['Alcance_x_m'].max():.1f} m \
        a {df_filtrado['Alcance_x_m'].min():.1f} m")
    ...
    with Progress(
        ...
        task_sel = progress.add_task(
            "Selecionando pontos...", total=len(df_filtrado))
        ...
        while idx_busca < len(df_filtrado):
            ...
            if melhor_idx is not None:
                pontos_selecionados.append(
                    df_filtrado.iloc[melhor_idx].to_dict())
                progress.update(task_sel, description=f"Ponto {len(pontos_selecionados)} | "
                                f"Elev {df_filtrado.iloc[melhor_idx]['Elevacao_deg']:.1f}° | "
                                f"Δ {melhor_diferenca:.0f}m"
                                )
    ...
    print(
        f"Faixa de elevação: {df_selecionados['Elevacao_deg'].max():.1f}°\
          a {df_selecionados['Elevacao_deg'].min():.1f}°")
    print(
        f"Faixa de alcance: {df_selecionados['Alcance_x_m'].max():.1f} m\
          a {df_selecionados['Alcance_x_m'].min():.1f} m")
    ...
    for i in range(1, len(df_selecionados)):
        dif = abs(df_selecionados.iloc[i]['Alcance_x_m'] -
                  df_selecionados.iloc[i-1]['Alcance_x_m'])
    ...
    for i in range(1, len(df_selecionados)):
        df_selecionados.loc[df_selecionados.index[i], 'Diferenca_x_m'] = \
            abs(df_selecionados.iloc[i]['Alcance_x_m'] -
                df_selecionados.iloc[i-1]['Alcance_x_m'])

    # Salvar em Excel e CSV
    excel_output = 'excels/pontos_selecionados_100m.xlsx'
    csv_output = 'excels/pontos_selecionados_100m.csv'

    df_selecionados.to_excel(excel_output, index=False, engine='openpyxl')
    df_selecionados.to_csv(csv_output, index=False)

    print("✓ Pontos salvos em:")
    ...
    print(f"{'='*80}")
    print(f"{'#':>3s} | {'Elevação':>9s} | {'Azimute':>10s} | {'Alcance X':>11s}\
           | {'Diferença':>10s} | {'Desvio Z':>9s}")
    ...
    for i in range(1, len(df_selecionados)):
        ax1.plot([df_selecionados.iloc[i-1]['Elevacao_deg'],
                  df_selecionados.iloc[i]['Elevacao_deg']],
                 [df_selecionados.iloc[i-1]['Alcance_x_m']/1000,
                     df_selecionados.iloc[i]['Alcance_x_m']/1000],
                 'r--', linewidth=1, alpha=0.5)

    ax1.set_xlabel('Elevação [°]', fontsize=12)
    ax1.set_ylabel('Alcance X [km]', fontsize=12)
    ax1.set_title('Alcance vs Elevação\n(Pontos com ~100m de espaçamento)',
                  fontsize=13, fontweight='bold')
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=10)
    ax1.axvline(x=elevacao_final, color='green', linestyle='--',
                linewidth=2, alpha=0.5, label=f'Alvo: {elevacao_final}°')
    ...
    if len(df_selecionados) > 1:
        ...
        _elevacoes_plot = df_selecionados['Elevacao_deg'].iloc[1:].values
        ...
        ax2.axhline(y=120, color='orange', linestyle=':',
    ...
    ax3.axvline(x=elevacao_final, color='green',
                linestyle='--', linewidth=2, alpha=0.5)

    # Gráfico 4: Vista de cima - Pontos de impacto
    ax4 = axes[1, 1]
    scatter = ax4.scatter(df_selecionados['Alcance_x_m']/1000,
                          df_selecionados['Desvio_z_resultante_m'],
    ...
    plt.suptitle('Análise de Pontos Selecionados (Espaçamento ~100m)\n' +
                 f'Total: {len(df_selecionados)} pontos |\
                   Elevação: {df_selecionados["Elevacao_deg"].max():.1f}°\
                     a {df_selecionados["Elevacao_deg"].min():.1f}°',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('images/pontos_selecionados_100m.png',
                dpi=150, bbox_inches='tight')
    print("✓ Visualização salva em: pontos_selecionados_100m.png")
    ...
    print(f"{'='*80}")
    print(
        f"Pontos totais disponíveis ({elevacao_inicial}° a {elevacao_final}°): {len(df_filtrado)}")
    print(f"Pontos selecionados (espaçamento ~100m): {len(df_selecionados)}")
    print(
        f"Taxa de seleção: {len(df_selecionados)/len(df_filtrado)*100:.1f}%")
    ...
    print("3. pontos_selecionados_100m.png")
