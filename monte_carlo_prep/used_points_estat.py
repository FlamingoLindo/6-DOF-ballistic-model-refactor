...
def used_points_estat():
    ...
    try:
        ...
        print(f"\nArquivo carregado: {len(df)} pontos")

        # INVERTER A ORDEM PARA FICAR CRESCENTE (menor alcance -> maior alcance)
        df = df.iloc[::-1].reset_index(drop=True)
        print(f"Ordem invertida: menor alcance ({df['Alcance_x_m'].iloc[0]/1000:.2f} km)\
               -> maior alcance ({df['Alcance_x_m'].iloc[-1]/1000: .2f} km)")
    ...
    _fig1, ax1 = plt.subplots(figsize=(16, 10), facecolor=colors['bg'])
    ax1.set_facecolor(colors['bg'])

    # Configurar estilo
    ax1.grid(True, alpha=0.3,
             color=colors['grid'], linestyle='-', linewidth=0.8)
    ...
    # Labels
    ax1.set_xlabel('Alcance [km]', fontsize=16,
                   fontweight='bold', color=colors['text'])
    ax1.set_ylabel('Desvio Lateral [m]', fontsize=16,
                   fontweight='bold', color=colors['text'])
    ...
    cbar.set_label('Sequencia de Pontos', fontsize=13,
                   fontweight='bold', color=colors['text'])
    ...
    # Ajustar limites
    x_margin = (df['Alcance_x_m'].max() -
                df['Alcance_x_m'].min()) * 0.08 / 1000
    ...
    plt.tight_layout()
    plt.savefig('images/grafico_1_vista_cima.png', dpi=300,
    ...
    if len(df) > 1:
        ...
        print(f"Distancias calculadas: min={diferencas.min():.1f}m,\
               max={diferencas.max():.1f}m, media={diferencas.mean():.1f}m")

        _fig2, ax2 = plt.subplots(figsize=(16, 9), facecolor=colors['bg'])
        ax2.set_facecolor(colors['bg'])

        # Configurar estilo
        ax2.grid(True, alpha=0.3,
                 color=colors['grid'], linestyle='-', linewidth=0.8, axis='y')
        ...
        # Barras
        _bars = ax2.bar(indices, diferencas, width=0.7, color=cores,
        ...
        # Labels
        ax2.set_xlabel('Intervalo entre Pontos', fontsize=16,
                       fontweight='bold', color=colors['text'])
        ax2.set_ylabel('Distancia [m]', fontsize=16,
                       fontweight='bold', color=colors['text'])
        ...
        plt.tight_layout()
        plt.savefig('images/grafico_2_distancias.png', dpi=300,
                    facecolor=colors['bg'], bbox_inches='tight')
    ...
    if len(df) > 1:
        _fig3, ax3 = plt.subplots(figsize=(14, 9), facecolor=colors['bg'])
        ...
        _counts, bins, patches = ax3.hist(diferencas, bins=n_bins,
        ...
        # Labels
        ax3.set_xlabel(
            'Distancia entre Pontos [m]', fontsize=16, fontweight='bold', color=colors['text'])
        ...
        stats_text = f'N = {len(diferencas)}\nMedia = {media:.1f} m\nDesvio Padrao = \
            {diferencas.std():.1f} m\nMin = {diferencas.min():.1f} m\nMax = {diferencas.max():.1f} m'
        ...
        plt.tight_layout()
        plt.savefig('images/grafico_3_histograma.png', dpi=300,
                    facecolor=colors['bg'], bbox_inches='tight')
    ...
    print(
        f"Alcance: {df['Alcance_x_m'].min()/1000:.2f} km a {df['Alcance_x_m'].max()/1000:.2f} km")
    print(
        f"Desvio lateral: {df['Desvio_z_resultante_m'].min():.2f} m\
              a {df['Desvio_z_resultante_m'].max():.2f} m")

    if len(df) > 1:
        diferencas_df = df['Alcance_x_m'].diff().dropna()
        print("\nEspacamento entre pontos:")
        print(f"Media: {diferencas_df.mean():.1f} m")
        print(f"Mediana: {diferencas_df.median():.1f} m")
        print(f"Minimo: {diferencas_df.min():.1f} m")
        print(f"Maximo: {diferencas_df.max():.1f} m")
        print(f"Desvio padrao: {diferencas_df.std():.1f} m")

        dentro_tolerancia = ((diferencas_df >= 80) &
                             (diferencas_df <= 120)).sum()
        print(
            f"Pontos dentro de +/-20m de 100m: {dentro_tolerancia}/{len(diferencas_df)}\
                  ({100 * dentro_tolerancia / len(diferencas_df):.1f}%)")
