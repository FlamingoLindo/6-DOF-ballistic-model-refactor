...
# Carregar resultados
FILE_PATH = r'excels/intervalos_confianca_monte_carlo.xlsx'
...
ax.set_xlabel('Índice (ordenado por distância)',
...
plt.tight_layout()
plt.savefig('images/confidence_interval/grafico_1_drone_sea_baby2.png',
...
ax.axhline(y=0.5, color='gray', linestyle='--',
           linewidth=1.5, alpha=0.4, label='50%')
...
plt.tight_layout()
plt.savefig('images/confidence_interval/grafico_2_iris_paykan2.png',
            dpi=300, bbox_inches='tight')
...
ax.set_title('Comparação: Drone Sea Baby vs IRIS Paykan - Probabilidades com IC (95%)\n' +
             ...
             fontsize=16, fontweight='bold', pad=20)
ax.grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
ax.set_ylim([-0.02, min(1.02, max(df_sea_baby['ic_sup_prop'].max(),
            df_iris['ic_sup_prop'].max()) + 0.05)])
ax.set_xlim([-2, max(len(df_sea_baby), len(df_iris)) + 2])

# Linhas de referência
ax.axhline(y=0.1, color='red', linestyle='--',
           linewidth=1.5, alpha=0.3, label='10%')
ax.axhline(y=0.5, color='gray', linestyle='--',
           linewidth=1.5, alpha=0.3, label='50%')

ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)

# Informações comparativas
textstr = 'Drone Sea Baby:\n'
textstr += f'  Média = {df_sea_baby["p_hat"].mean()*100:.2f}%\n'
textstr += f'  Máximo = {df_sea_baby["p_hat"].max()*100:.2f}%\n\n'
textstr += 'IRIS Paykan:\n'
textstr += f'  Média = {df_iris["p_hat"].mean()*100:.2f}%\n'
...
plt.tight_layout()
plt.savefig('images/confidence_interval/grafico_3_comparacao_ambos2.png',
            dpi=300, bbox_inches='tight')
...
print(
    f"Distância: {df_sea_baby['Alcance_m'].min():.1f}m - {df_sea_baby['Alcance_m'].max():.1f}m")
...
print(
    f"Distância: {df_iris['Alcance_m'].min():.1f}m - {df_iris['Alcance_m'].max():.1f}m")
...
print(
    f"IRIS Paykan tem taxa média {ratio_media:.1f}x maior que Drone Sea Baby")
