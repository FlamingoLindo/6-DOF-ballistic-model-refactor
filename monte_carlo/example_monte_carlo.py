...
def eg_monte_carlo():
    ...
    df_pontos_filtrados = df_pontos_filtrados.sort_values(
        'Elevacao_deg', ascending=False)
    ...
    # Perturbações nos ângulos de tiro
    delta_elevacao_todas = np.random.normal(
        0, sigma_elevacao, n_simulacoes_total)
    delta_azimute_todas = np.random.normal(
        0, sigma_azimute, n_simulacoes_total)
    ...
    ax1.set_title('Comparação: Todas as Embarcações',
                  fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Comparação: Todas as Embarcações vs Alcance',
                  fontsize=14, fontweight='bold')
